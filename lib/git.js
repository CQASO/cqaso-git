...
function _gitCommit(message, callback) {
...
function _gitPush(callback) {
    let command = `git push`
    console.log(`>>> Exec ${command} at ${process.cwd()}`);
    childProcess.execSync('git config --local push.default current');
    childProcess.exec(command, function(err, stdout, stderr){
        if(!err){
            stdout = stderr;
            stderr = null;
...
function _changePackage(tag, callback){
    ...
    if(stats.isFile()){
        ...
        childProcess.execSync('git config --local push.default current');
        let command = `git add package.json && git commit -m \'chore(package.json): updaten version to ${tag} by cqGit\' && git push`;
